...
<form action="{{ route('admin.dish.store') }}" method="POST" class="max-w-sm mx-auto">
    ...
    <div class="mb-5">
        ...
        <input type="text" name="dishnumber" id="dishnumber" value="{{ old('dishnumber') }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"/>
        @error('dishnumber')
    ...
    <div class="mb-5">
        ...
        <input type="text" name="name" id="name" value="{{ old('name') }}" required class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"/>
        @error('name')
            <p class="text-red-500"> {{ $message }} </p>
        @enderror
    </div>
    ...
    <div class="mb-5">
        ...
        <select name="dishtype" id="dishtype" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
            @foreach ($dishtypes as $dishtype)
                <option value="{{ $dishtype->type }}" @if (old('dishtype') == $dishtype->type) selected @endif>
                    {{ $dishtype->type }}
                </option>
            @endforeach
        </select>
    ...
    <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Gerecht aanmaken</button>
  </form>
@endsection
...
@section('content')
    <div id="app" class="flex h-[calc(100vh-15rem)]"> <!-- Adjusting for any potential header/footer -->
        <!-- Left div: List of dishes -->
        <div class="w-1/2 bg-gray-100 border-2 border-black overflow-y-scroll pr-4 pt-4">
            <input class="w-1/2" type="text" id="searchInput" placeholder="Zoek op gerechtnaam of nummer...">
            @foreach($dishes->groupBy('dishtype') as $type => $dishesByType)
                @if($dishesByType->isNotEmpty()) <!-- Check if dishes exist under this type -->
                    <h2 class="text-xl font-semibold my-4 px-4">{{ $type }}</h2>
                    <ul>
